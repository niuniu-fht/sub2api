...
type Group struct {
	...
	// OpenAI 图片按 quality×size 的按次单价（USD）；key 为 low/medium/high，value 为 1K/2K/4K→价格
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case group.FieldImageQualityPrices, group.FieldVideoModelPrices, group.FieldModelPricing, group.FieldModelRouting, group.FieldSupportedModelScopes, group.FieldMessagesDispatchModelConfig, group.FieldModelAllowlist, group.FieldCodexModelsManifestConfig, group.FieldReasoningEffortMappings:
			values[i] = new([]byte)
	...
	for i := range columns {
		...
		case group.FieldImageQualityPrices:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field image_quality_prices", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.ImageQualityPrices); err != nil {
					return fmt.Errorf("unmarshal field image_quality_prices: %w", err)
				}
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("image_quality_prices=")
	builder.WriteString(fmt.Sprintf("%v", _m.ImageQualityPrices))
	builder.WriteString(", ")
